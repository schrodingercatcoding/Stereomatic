...
def prepare_data_p(data1, data2):
    """
    A function to make data1 and data2 same dimension in order to do overlap calculation
...
def prepare_data_again(data1, data2):
    ...
    for key1, key2 in zip(keys_1, keys_2):
        ...
        for value1, value2 in zip(data1[key1], data2[key2]):
            obj1 = value1[1]
            obj2 = value2[1]
            obj1_p, obj2_p = prepare_data_p(obj1, obj2)
...
def calculate_overlap_layer1(data1, data2):
    ...
    for key1, key2 in zip(data1.keys(), data2.keys()):
        arr1 = [ data1[key1][i][0][0] for i in range(len(data1[key1]))] 
        arr2 = [ data2[key2][i][0][0] for i in range(len(data2[key2]))] 
...
def calculate_all_over_lap(data1, data2):
    ...
    data1_pack, data2_pack = prepare_data(data1, data2)
    difference_1 = calculate_overlap_layer1(data1_pack, data2_pack)
    overlap1 = np.exp(-difference_1)

    # pack data to the same second layer and calculate dot_product
    data1_pack_pack, data2_pack_pack = prepare_data_again(data1_pack, data2_pack)
    difference_2 = calculate_overlap_layer2(data1_pack_pack, data2_pack_pack)
...
if __name__ == "__main__":
    ...
    with open(sys.argv[1].replace('.json', '_pack.json'), 'w') as fh:
        json.dump(data1_pack, fh, indent=4)
    with open(sys.argv[2].replace('.json', '_pack.json'), 'w') as fh:
        json.dump(data2_pack, fh, indent=4)
    ...
    with open(sys.argv[1].replace('.json', '_pack_pack.json'), 'w') as fh:
        json.dump(data1_pack_pack, fh, indent=4)
    with open(sys.argv[2].replace('.json', '_pack_pack.json'), 'w') as fh:
        json.dump(data2_pack_pack, fh, indent=4)
